...
Here’s a polished `ANSWERS.md` you can directly place in your repo root.
...
There are also `node_modules` folders present locally during development, but they are not required in the repository because all dependencies can be installed using:
...
because the task required persistent CRUD operations and authentication, and this stack is very suitable for quickly building APIs and CLI-based applications.

MongoDB worked well because blog data is document-oriented and flexible. Mongoose also simplified schema validation and relationships between users and blogs.

I used a CLI interface because it is lightweight, easy to test on a fresh machine, and focuses more on backend logic and persistence rather than frontend styling.

A worse choice would have been using plain filesystem storage with JSON files because:

* concurrency becomes difficult
* querying/searching blogs is inefficient
* relationships between users and blogs become harder to manage
* authentication logic becomes messy

Another worse choice would have been building a full frontend framework for this small task because it would add unnecessary complexity and setup overhead.
...
Preventing unauthenticated users from creating or modifying blogs.
...
Before protected actions such as:
...
the CLI checks whether the user is logged in.
...
This prevents invalid API requests from being sent.

Without this handling:

* users would repeatedly hit protected endpoints
* backend would return 401 errors
* CLI would display large Axios error objects
* overall UX would become confusing

Another important edge case handled in the backend is password validation inside the Mongoose schema:

```js
/^(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$/
```

This ensures weak passwords cannot be stored.

# 4. AI Usage

I used ChatGPT free version during development for:
...
But this prevented public blog searching and viewing which is not common anyone can view any blog or search for blogs.

So I changed the routing structure so public routes like:
...
* "Why is Axios returning 401 even after login?"
* "How to attach JWT token using Axios interceptors?"
* "How to structure a CLI menu using Inquirer?"
* "How to improve Mongoose aggregate lookup?"
